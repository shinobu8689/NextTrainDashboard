...
from Cython.Distutils.old_build_ext import show_compilers
...
def main(input_string, conn):
    ...
    for each in rows:
        ...
        print(  f"{colour_text(route_color, "█")} {platform:<3}  "
                f"{headsign:<35}"
                f"{dep_time_display:>15} {mins_display} {delay_str} {trip_id if show_debug else ""} {block_id if show_debug else ""}"
        )
